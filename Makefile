...
app: $(SWIFT_FILES)
	...
	@/usr/libexec/PlistBuddy -c "Add :CFBundleVersion string 1.1.3" "$(BUILD_DIR)/$(BUNDLE)/Contents/Info.plist" 2>/dev/null || true
	@/usr/libexec/PlistBuddy -c "Add :CFBundleShortVersionString string 1.1.3" "$(BUILD_DIR)/$(BUNDLE)/Contents/Info.plist" 2>/dev/null || true
